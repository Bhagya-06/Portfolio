...
export const Timeline = ({ data }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const threshold = window.innerHeight * 0.45; // Triggers active highlight as path line draws down
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className="timeline-item flex justify-start pt-10 md:pt-40 md:gap-10 animate-fadeUp"
            >
              <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
                <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-card border border-border flex items-center justify-center">
                  <div
                    className={`h-4 w-4 rounded-full border transition-all duration-300 p-2 ${
                      isActive
                        ? "bg-primary border-primary scale-125 shadow-[0_0_10px_rgba(45,212,191,0.6)]"
                ...
                <h3
                  className={`hidden md:block text-xl md:pl-20 md:text-5xl font-bold transition-colors duration-300 ${
                    isActive ? "text-white" : "text-muted-foreground/35"
                  }`}
                >
                  {item.title}
                </h3>
